...
public class AntSystemDensity {
	...
	public AntSystemDensity(ArrayList<NodeModel> nodesList, double[][] neighborsMatrix, int startIndex, int endIndex,
...
public void run(){
    	...
    	double roadLenght = Double.MAX_VALUE;
    		...
    		while(!allAntsFoundEnd){
    			...
    			for(Ant ant: ants){
    				...
    				for(int nodeIndex = 0; nodeIndex < visibilityMultPheromones.length; nodeIndex++){
    					if(neighborsMatrix[ant.getTabuList().get(ant.getTabuList().size()-1)][nodeIndex] > 0){
    						if(ant.getTabuList().contains(nodeIndex) || ant.getWithdrawnList().contains(nodeIndex)){
    							continue;
    						}
    						ant.addNewNodeCandidate(nodeIndex, calculateVisibilityMultPheromone(ant.getTabuList().get(ant.getTabuList().size()-1), nodeIndex, pheromonesMatrix, visibilityMatrix));
    				...
    				if(ant.getProbablisticNodesList().size() == 0){
    					...
    					allAntsFoundEnd=false;
    					pheromonesBackwardsMinus(ant.getTabuList(), ant.getWithdrawnList(), pheromonesMatrix);
    				}
    				else{
        				...
        				phermonesLocalMinus(ant.getTabuList(), pheromonesMatrix);
        				updatePheromonsPlus(ant.getTabuList(), pheromonesMatrix, pheromonesPlus);
    				}
...
//    			antsBackwardsList.clear();
    		...
    		for(Ant ant: ants){
    			...
    			if(roadLenght > antRoadLength){
    				roadLenght = antRoadLength;
    				road = new ArrayList<Integer>(ant.getTabuList());
    			}
    		}
    		paintAllEdgesWithActualPheromones(pheromonesMatrix);
    	}
    	paintRoadChosenByAnts(pheromonesMatrix);
    	System.out.println(roadLenght);
    }

	private void phermonesLocalMinus(ArrayList<Integer> tabuList, double [][] pheromonesMatrix){
		pheromonesMatrix[tabuList.get(tabuList.size()-1)][tabuList.get(tabuList.size()-2)] = pheromonesMatrix[tabuList.get(tabuList.size()-1)][tabuList.get(tabuList.size()-2)] * (1 - pheromonesMinus);
		pheromonesMatrix[tabuList.get(tabuList.size()-2)][tabuList.get(tabuList.size()-1)] = pheromonesMatrix[tabuList.get(tabuList.size()-1)][tabuList.get(tabuList.size()-2)];
	}
	...
	private double calculateVisibilityMultPheromone(int row, int column, double [][] pheromonesMatrix, double [][] visibilityMatrix){
		if(pheromonesMatrix[row][column] > 0){
			return BigDecimal.valueOf(Math.pow(visibilityMatrix[row][column], beta)).multiply(BigDecimal.valueOf(Math.pow(pheromonesMatrix[row][column], alfa))).doubleValue();
		}
		else{
			return BigDecimal.valueOf(Math.pow(visibilityMatrix[row][column], beta)).doubleValue();	
		}
	}
